...
DEFINE_BOOL(tabBarReflectsSessionBackgroundColor, NO, SECTION_TABS @"Tab bar reflects each tab’s terminal background color?\nWhen enabled, a tab in the tab bar takes on the session’s current terminal background color (the one set by OSC 11, profile, or theme) when no explicit tab color is assigned. Tabs with an explicitly-set tab color are unaffected. Most visible in the Minimal theme.");
